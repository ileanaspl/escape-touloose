...
export function quizz() {
  ...
  function displayQuestions() {
    interrogation.innerText =
      currentQuestionIndex === 3 ? "ici le bug" : questions[currentQuestionIndex].question;
    if (currentQuestionIndex === 3) {
      if (score === 3) {
        interrogation.innerText =
          "Bravo, vous avez réussi cette étape, rendez vous dans le prochain quartier ";
        setTimeout(() => {
          raz();
          welcomeInTheNeighborhood(
            "dans le quartier de Saint-Sernin, Arnaud-Bernard",
            coupsdemidi,
            1,
          );
        }, 2500);
  ...
  function repondre(selectedAnswer) {
    ...
    if (choice === question.reponse) {
      ...
      choixRéponse.innerHTML = "";
      interrogation.innerText =
        "Dommage ! avez-vous pensé à ouvrir wikipédia ?! \n Direction le trou ! ";
      setTimeout(() => {
        raz();
